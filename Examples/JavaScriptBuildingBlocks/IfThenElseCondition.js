// We are using the rpio library to install it go to command prompt and type "npm install rpio"

////////////////////////////////
// Initialization Section     //
///////////////////////////////

// Create an object that points to the 'rpio' library 
var rpio = require('rpio');

// Create objects that represent pin numbers to inputs and outputs on our Raspberry PI to make our code easier to read and change.

// Inputs
var Switch1_Top = 31;
var Switch2_Left = 33;
var Switch3_Bottom = 35;
var Switch4_Right = 37;

// Define the direction of the Input pins above
rpio.open(Switch1_Top, rpio.INPUT);
rpio.open(Switch2_Left, rpio.INPUT);
rpio.open(Switch3_Bottom, rpio.INPUT);
rpio.open(Switch4_Right, rpio.INPUT);

// Outputs
var RedLED = 32;
var GreenLED = 36;
var BlueLED = 38;
var Buzzer = 40;

// Define the direction and initial state of the output pins above.
rpio.open(RedLED, rpio.OUTPUT, rpio.HIGH);
rpio.open(GreenLED, rpio.OUTPUT, rpio.HIGH);
rpio.open(BlueLED, rpio.OUTPUT, rpio.HIGH);
rpio.open(Buzzer, rpio.OUTPUT, rpio.LOW);


// Let's blink an LED but this time change the color if Switch1 is pressed.  
// This is called a conditional statement
// This is a type of loop called a "ForNext Loop" It will only repeat the code within the curly brackets the exact number of times specified in this case 5.
console.log('Starting Conditional Statement Example');
for (var i = 0; i < 5; i++) 
{
        var ValueOfSwitch1 = rpio.read(Switch1_Top);

        // This is called an "if then else" conditional statement and will perform different functions depending on whether the statment within the () brackets is TRUE or FALSE.
        if (ValueOfSwitch1 == rpio.LOW)
        {
                rpio.write(BlueLED, rpio.LOW);
        }
        else
        {
                rpio.write(RedLED, rpio.LOW);
        }
        
        rpio.sleep(1);
        
        
        if (ValueOfSwitch1 == rpio.LOW)
        {
                rpio.write(BlueLED, rpio.HIGH);
        }
        else
        {
                rpio.write(RedLED, rpio.HIGH);
        }

        rpio.msleep(500);
}


// Let's blink an LED to until we press Switch1 to exit the loop.
// This is a type of loop called a "While Loop" It will only repeat the code within the curly brackets indefinitely until the statement within the () brackets is false.

console.log('Starting While Loop Example');
while (rpio.read(Switch1_Top) != rpio.LOW) 
{
        /* On for 1 second */
        rpio.write(RedLED, rpio.LOW);
        rpio.sleep(1);
        
        /* Off for half a second (500ms) */
        rpio.write(RedLED, rpio.HIGH);
        rpio.msleep(500);
}
